            ...
            <ul class="w-full xl:w-1/3">
                ...
                @foreach($languages as $language)
                    <li class="w-full text-left inline-block my-1.5">
                        <a href="{{ rute_to_language(collect(['name' => to_slug($language->name), 'id' => (int)$language->id ])) }}" class="border-b border-dotted border-blue-500 text-blue-800 hover:text-orange-700 font-bold">{{ $language->name }}</a>
                    </li>
